...
st.markdown("""
This webapp retrieves cryptocurrency prices for the top 100 cryptocurrency from the coinMarketCap
""")

# About
expander_bar = st.beta_expander("About")
expander_bar.markdown("""* **Python libraries:** base64, pandas, streamlit, numpy, matplotlib, seaborn, BeautifulSoup, requests, json, time
...
def load_data():
    ...
    data = soup.find('script', id='__Next_DATA__', type = 'application/jason')
    coins = {}
    coin_data = jason.loads(data.contents[0])
    ...
    df = pd.DataFrame(columns = ['coin_name', 'coin_symbol', 'marketCap', 'percentChange1h', 'percentChange24h', "pecentChane7d", 'price', 'volume24h'])
...
df_change["Positive_percent_change_ih"] = df_change['percentChange1h'] >0
...
if percent_timeframe == '7d':
    ...
    plt.figure(figsize=(5,25))
    plt.subplots_adjust(top =1, bottom=0)
    df_change['percentChange7d'].plot(kind='barh', color=df_change.positive_percent_change_24h.map({True: 'g', False: 'r'}))
    col3.pyplot(plt)
else:
    if sort_values == 'Yes':
